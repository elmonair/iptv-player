...
export default function SeriesCategories({ selectedCategoryId, onSelectCategory, sourceId }: Props) {
  ...
  const seriesCounts = useLiveQuery(
    async () => {
      const seriesList = await db.series.where('sourceId').equals(sourceId).toArray()
  ...
  const totalCount = useMemo(() => {
    if (!seriesCounts) return 0
    let total = 0
    seriesCounts.forEach((count) => { total += count })
    return total
  }, [seriesCounts])

  const handleKeyDown = useCallback((e: React.KeyboardEvent, _targetCategoryId: string | null, index: number) => {
    if (!categories) return
    const allItems = [{ id: null, name: 'All Series' }, ...categories]
    let nextIndex = index
    ...
    if (nextIndex !== index) {
      const items = [{ id: null }, ...(categories ?? [])]
      onSelectCategory(items[nextIndex].id)
    }
  ...
  const allItems = [
    { id: null, name: 'All Series', count: totalCount },
    ...(categories ?? []).map(c => ({ id: c.id, name: c.name, count: seriesCounts?.get(c.id) ?? 0 })),
  ]
  ...
  return (
    <div className="w-full h-full bg-slate-900 flex flex-col overflow-hidden">
      ...
      <nav className="flex-1 overflow-y-auto min-h-0" aria-label="Series categories">
        {categories.length === 0 ? (
